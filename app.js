(function () {
  ...
  function parseTerminalResponses(text) {
    return String(text || "")
      .split(/\r?\n/)
      .map((line) => line.trim())
  ...
  function loadTerminalResponses() {
    ...
    return fetch("terminal-responses.txt", { cache: "no-store" })
      .then((response) => {
        if (!response.ok) throw new Error("terminal response file unavailable");
        return response.text();
      })
      .then((text) => {
        const loadedResponses = parseTerminalResponses(text);
        if (loadedResponses.length) terminalResponses = loadedResponses;
      })
      .catch(() => {
        terminalResponses = fallbackTerminalResponses.slice();
      });
  }
